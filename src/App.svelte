<script>
  ...
  import {
    ...
    nextExponential,
    nextNormal
  } from "./services/randomNumbers.js";
  ...
  $: tempoMedioDeServicos = () => {
    ...
    let tempoTotalDeServicos = listaDeTemposDeServicos.reduce(
      (acc, atual) => (acc += atual)
    );
  ...
  $: tempoMedioDeEsperaNaFila = () => {
    ...
    let tempoDeEsperaTotal = listaDeTemposDeEspera.reduce(
      (acc, atual) => (acc += atual)
    );
  ...
  $: probabilidadeDeEspera = () => {
    let numeroTotalDeClientes = numeroDeClientesAcumulados;
    let listaDeEsperasPelosClientes = servicos.map(
      servico => servico.tempoClienteNaFila
    );
    let numeroDeClientesQueEsperaram = listaDeEsperasPelosClientes.filter(
  ...
  $: probabilidadeDeOperadorLivre = () => {
    let listaDeTemposLivresDosOperadores = servicos.map(
      servico => servico.tempoLivreDoOperador
    );
    let totalDeTempoLivreDosOperadores = listaDeTemposLivresDosOperadores.reduce(
      (acc, atual) => (acc += atual)
    );
  ...
  $: tempoMedioDespendidoNoSistema = () => {
    let numeroTotalDeClientes = numeroDeClientesAcumulados;
    let listaDeTemposNoSistema = servicos.map(
      servico => servico.tempoClienteNoSistema
    );
    let totalDeTemposNoSistema = listaDeTemposNoSistema.reduce(
      (acc, atual) => (acc += atual)
    );
  ...
  const gerarNaoPrimeiroServico = (NTDUC, NTSNR) => {
    ...
    const novoTempoChegadaNoRelogio =
      ultimoServico().tempoChegadaNoRelogio + novoTempoDesdeUltimaChegada;
    ...
    const novoTempoInicioServicoNoRelogio = () => {
      const tempoDoUltimoServicoJaConcluido = ultimoServico()
      ...
      else return novoTempoChegadaNoRelogio;
    };

    const novoTempoClienteNaFila = () => {
      const diferencaEntreTempos =
        ultimoServico().tempoFinalDoServicoNoRelogio -
        novoTempoChegadaNoRelogio;
      if (diferencaEntreTempos > 0) return diferencaEntreTempos;
      else return 0;
    };

    const novoTempoFinalDoServicoNoRelogio =
      novoTempoDeServico + novoTempoInicioServicoNoRelogio();
    const novoTempoClienteNoSistema =
      novoTempoDeServico + novoTempoClienteNaFila();
    const novoTempoLivreDoOperador =
      novoTempoInicioServicoNoRelogio() -
      ultimoServico().tempoFinalDoServicoNoRelogio;
  ...
  const gerarServico = (NTDUC, NTSNR) => {
    ...
    else novoServico = gerarPrimeiroServico(NTDUC, NTSNR);
  ...
  const simularProblema = () => {
    ...
    while (tempoAtualDaSimulação < tempoTotalDeSimulacao) {
      ...
      let novoServico = gerarServico(
        temposEntreTodasChegadas[i],
        temposDeTodosOsServicos[i]
      );
  ...
  const handleSimularSlideClick = () => {
    ...
    for (let i = 0; i < numeroDeServicos; i++) {
      setTimeout(() => {
        let novoServico = gerarServico(
          SIMULATION_EXAMPLE.temposEntreTodasChegadas[i],
          SIMULATION_EXAMPLE.temposDeTodosOsServicos[i]
        );
        servicos = [...servicos, novoServico];
        alimentarDadosDosGraficos();
      }, 1000 * i);
...
<form class="container my-5" on:submit={handleSubmit}>
  <div class="form-row">
    <div class="form-group col-md-4 offset-md-2">
      <label>
        Tempo da simulação (em minutos)
        <strong>(max 24 hrs)</strong>
      </label>
      <input
        required
        bind:value={tempoTotalDeSimulacao}
        type="number"
        min="1"
        max="1440"
        class="form-control"
        placeholder="tempo da simulação" />
    </div>
  </div>
  ...
  <div class="form-row">
    <div class="form-group col-md-4 offset-md-2">
      ...
      <select
        required
        class="form-control"
        value={tecSelecionado}
        bind:value={tecSelecionado}>
        <option value={0}>Selecione alguma...</option>
  ...
  <div class="form-row">
    ...
    {#if tsSelecionado.id === 1}
      <div in:fade class="form-group col-md-4">
        <label for="inputNumeroDeServicos">
          Quantos minutos o serviço demora?
        </label>
        <input
          type="number"
          bind:value={inputMeanTS}
          class="form-control"
  ...
  <div class="offset-md-2">
    ...
    <button
      class="btn btn-large btn-danger float-right"
      on:click|preventDefault={handleSimularSlideClick}>
      Simular Exemplo Slide
    </button>
  </div>
...
<div class="container">
  <div class="d-flex justify-content-end text-right">
    <button
      class="btn-imprimir btn btn-secondary"
  ...
  <div class="row">
    <div class="col-md-4">
      <ChartSimulacao
        data={listaDeTemposMediosNafila}
        backgroundColor="#EF5B5B"
        numeroDeIteracoes={numeroDeServicos || 30}
        titulo="Tempo médio na fila de espera" />
    </div>

    <div class="col-md-4">
      <ChartSimulacao
        data={listaDeTemposMediosDeServico}
        backgroundColor="#BAA5FF"
        numeroDeIteracoes={numeroDeServicos || 30}
        titulo="Tempo médio de serviço" />
    </div>
        ...
        <div transition:fade={{ duration: 1000 }} class="pt-4">
          <p>
            ...
            <span class="bg-primary p-1 text-light rounded">
              {numeroDeClientesAcumulados}
            </span>
          </p>
          <p>
            Tempo médio de espera na fila:
            <span class="bg-primary p-1 text-light rounded">
              {tempoMedioDeEsperaNaFila().toFixed(2)}
            </span>
          </p>
          <p>
            Probabilidade de um cliente esperar na fila:
            <span class="bg-primary p-1 text-light rounded">
              {probabilidadeDeEspera().toFixed(2)}
            </span>
          </p>
          <p>
            Probabilidade do operador livre:
            <span class="bg-primary p-1 text-light rounded">
              {probabilidadeDeOperadorLivre().toFixed(2)}
            </span>
          </p>
          <p>
            Tempo médio de serviço:
            <span class="bg-primary p-1 text-light rounded">
              {tempoMedioDeServicos().toFixed(2)}
            </span>
          </p>
          <p>
            Tempo médio despendido no sistema:
            <span class="bg-primary p-1 text-light rounded">
              {tempoMedioDespendidoNoSistema().toFixed(2)}
            </span>
          </p>
        </div>
      {/if}
  ...
  <div class="row">
    <div class="col-md-4">
      <ChartSimulacao
        data={listaDeProbabilidadesDeOperadoresLivre}
        backgroundColor="#466365"
        numeroDeIteracoes={numeroDeServicos || 30}
        titulo="Probabilidade de operador livre" />
    </div>

    <div class="col-md-4">
      <ChartSimulacao
        data={listaDeProbDeClienteEsperarNaFila}
        backgroundColor="#ABFAA9"
        numeroDeIteracoes={numeroDeServicos || 30}
        titulo="Probabilidade do cliente esperar" />
    </div>

    <div class="col-md-4">
      <ChartSimulacao
        data={listaDeTemposMediosDespendidoNoSistema}
        backgroundColor="#C6B9CD"
        numeroDeIteracoes={numeroDeServicos || 30}
        titulo="Tempos Médios despendido no sistema" />
    </div>
  </div>
